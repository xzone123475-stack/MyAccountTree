...
function applyZoomAt(screenX, screenY, newZoom) {
  ...
  zoom = newZoom;
  offsetX = screenX - width / 2 - worldBefore.x * zoom;
  offsetY = screenY - height / 2 - worldBefore.y * zoom;
}
...
function touchMoved() {
  if (touches.length === 2) {
    ...
    if (lastTouchDist !== null) {
      ...
      if (newZoom >= prevZoom) {
        // 🔥 확대할 때는 손가락 기준 유지
        applyZoomAt(midX, midY, newZoom);
      } else {
        // 🔥 축소할 때는 중앙 기준으로 줌
        zoom = newZoom;

        // zoom이 1에 가까워질수록 offset도 자연스럽게 0으로 복귀
...
function mouseWheel(event) {
  ...
  if (newZoom >= prevZoom) {
    applyZoomAt(mouseX, mouseY, newZoom);
  } else {
    zoom = newZoom;
    const centerRatio = map(zoom, 0.3, 1.0, 1.0, 0.0, true);
    offsetX *= centerRatio;
